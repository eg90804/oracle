...
class OraDaemon < EasyType::Daemon
  ...
  def self.run(user, sid, oraUser='sysdba', oraPassword=nil)
    ...
    if daemon
      ...
      new(user, sid, oraUser, oraPassword)
    end
  end

  def initialize(user, sid, oraUser, oraPassword)
    @user = user
    @sid = sid
    @oraUser = oraUser
  ...
  def execute_sql_command(command, output_file, debug = false)
    ...
    sync {|line| fail "Error in execution of SQL command: #{command}.\nFound error #{line}" if line =~ ORACLE_ERROR}
  end
