...
module Puppet
  newtype(:oracle_service) do
    ...
    on_create do
      sql "exec dbms_service.create_service('#{name}', '#{name}'); dbms_service.start_service('#{name}')"
    ...
    on_destroy do
      sql  "exec dbms_service.stop_service('#{name}'); dbms_service.delete_service('#{name}')"
